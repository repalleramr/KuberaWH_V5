...
function startPrayoga(){ if(state.currentChakra===0 && !(currentKumbh()?.rows?.length)){ state.liveBankroll = state.settings.bankroll; } else if(state.currentChakra!==0 || currentKumbh()?.rows?.length){ state.currentKumbhId=null; state.liveBankroll = state.settings.bankroll; state.currentChakra=0; state.numbers={Y:createSide(),K:createSide()}; state.drishti=[]; state.summary={totalAhuti:0,maxExposure:0}; pending={Y:null,K:null}; historyStack=[]; redoStack=[]; if(state.pendingAttackMode){ state.settings.attackMode = state.pendingAttackMode; state.pendingAttackMode = null; } } const kumbh=ensureKumbh(); if(state.currentChakra===0 && !(currentKumbh()?.rows?.length) && state.pendingAttackMode){ state.settings.attackMode = state.pendingAttackMode; state.pendingAttackMode = null; } historyStack=[]; redoStack=[]; state.activeTab='sangram'; renderAll(); showToast('SANGRAM AARAMBHA', `#${String(kumbh.id).padStart(2,'0')} Kumbh ready`); }
async function clearCurrentSession(){ if(!(await askClearKumbh())) return; state.liveBankroll=state.settings.bankroll; state.currentChakra=0; state.numbers={Y:createSide(),K:createSide()}; state.drishti=[]; state.summary={totalAhuti:0,maxExposure:0}; pending={Y:null,K:null}; historyStack=[]; redoStack=[]; state.currentKumbhId=null; if(state.pendingAttackMode){ state.settings.attackMode = state.pendingAttackMode; state.pendingAttackMode = null; } const kumbh=ensureKumbh(); state.activeTab='sangram'; renderAll(); showToast('KUMBHA SHUDDHI',`#${String(kumbh.id).padStart(2,'0')} Kumbh ready`); }
...
async function resolveNumber(side,num,notes,rowEvents){ const info=state.numbers[side][num]; if(info.status==='L'){ return; }
  if(info.status==='C'){
    ...
    info.status='B'; info.ladder=2; info.step=1; info.pendingSecond=false; info.watchCount=0; if(rowEvents) rowEvents.ret.push(`${side}${num}`); notes.push({title:'CAP RETURNED',text:`${side}${num} back on track`,kind:'warn'}); return;
  }
  if(info.status==='I'){
    info.status='A'; info.step=0; info.ladder=1; info.activeAt=state.currentChakra; info.prevLoss=0; info.watchCount=0; return;
  }
...
async function applyYantraSettings(){
  ...
  const prevAttackMode = state.pendingAttackMode || state.settings.attackMode || 'classic';
  const nextSettings = readYantraSettings();
  const nextAttackMode = nextSettings.attackMode || 'classic';
  state.settings = nextSettings;
  if(hasCurrentKumbhProgress() && prevAttackMode !== nextAttackMode){
    state.pendingAttackMode = nextAttackMode;
    state.settings.attackMode = prevAttackMode
  } else {
    state.pendingAttackMode = null;
    state.settings.attackMode = nextAttackMode;
  }
  ...
  renderAll();
  showToast('YANTRA APPLIED', state.pendingAttackMode ? `Settings updated. ${state.pendingAttackMode==='fourthstrike'?'FourthStrike':state.pendingAttackMode==='thirdstrike'?'ThirdStrike':'Classic'} starts from next Kumbh` : 'Settings updated');
}
